...
local function TurnOff(inst, instant)
    inst.on = false
    inst.components.friendlyfiredetector:Deactivate()

    if not inst:HasTag("fueldepleted") then 
        local randomizedStartTime = POPULATING
        inst:DoTaskInTime(0, inst.components.friendlyfiredetector:ActivateEmergencyMode(randomizedStartTime)) -- this can be called from onload, so make sure everything is set up first
    end
...
local function TurnOn(inst, instant)
    inst.on = true
    if not inst.components.friendlyfiredetector:IsEmergency() then
        local randomizedStartTime = POPULATING
        inst.components.friendlyfiredetector:Activate(randomizedStartTime)
...
local function fn()
    ...
    inst:AddComponent("friendlyfiredetector")
    inst.components.friendlyfiredetector:SetOnFindFireFn(OnFindFire)
    inst.components.friendlyfiredetector:SetOnBeginEmergencyFn(OnBeginEmergency)
    inst.components.friendlyfiredetector:SetOnEndEmergencyFn(OnEndEmergency)
    inst.components.friendlyfiredetector:SetOnBeginWarningFn(OnBeginWarning)
    inst.components.friendlyfiredetector:SetOnUpdateWarningFn(OnUpdateWarning)
    inst.components.friendlyfiredetector:SetOnEndWarningFn(OnEndWarning)
    ...
    inst.components.wateryprotection:AddIgnoreTag("campfire")
